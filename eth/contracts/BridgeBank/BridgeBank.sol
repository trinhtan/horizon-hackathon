...
contract BridgeBank is EthereumBank {
    ...
    function addToken(address _token, address _hmyToken) public onlyOperator {
        require(_token != address(0) && _hmyToken != address(0), "Invalid token");
     ...
     */
    function lockERC20(address _harmonyReceiver, address _ethereumToken, uint256 _amountEthereumToken)
        public
        availableNonce()
    {
        require(_ethereumToken != ETHAddress && tokenPairMaps[_ethereumToken] != address(0), "Invalid token");

        uint256 fee;

        if (feeNumerator != 0){
            fee = _amountEthereumToken.mul(feeNumerator).mul(SAFE_NUMBER).div(feeDenominator).div(SAFE_NUMBER);
        }

        require(
            IERC20(_ethereumToken).transferFrom(msg.sender, address(this), _amountEthereumToken + fee), "Contract token allowances insufficient to complete this lock request"
        );

        lockERC20Asset(msg.sender, _harmonyReceiver, _ethereumToken, _amountEthereumToken);
    }
        ...
        availableNonce()
    {
        ...
        if (feeNumerator != 0){
            fee = _amountETH.mul(feeNumerator).mul(SAFE_NUMBER).div(feeDenominator).div(SAFE_NUMBER);
        }

        require(msg.value == _amountETH + fee, "The transactions value must be equal the specified amount (in wei)");

        BandOracleInterface.ReferenceData memory data = bandOracleInterface.getReferenceData('ETH', 'ONE');
        uint256 amountONE = _amountETH.mul(data.rate);

        lockAndSwapETHForONE(msg.sender, _harmonyReceiver, _amountETH, amountONE);
    }

    function swapETHForToken(address _harmonyReceiver, uint256 _amountETH, string memory _destTokenSymbol)
        public
        payable
        availableNonce()
    {
        ...
        BandOracleInterface.ReferenceData memory data = bandOracleInterface.getReferenceData('ETH', _destTokenSymbol);
        uint256 amountHarmonyToken = _amountETH.mul(data.rate);

        lockAndSwapETHForToken(msg.sender, _harmonyReceiver, _amountETH, _destTokenSymbol, amountHarmonyToken);
    }

    function swapTokenForToken(address _harmonyReceiver, address _ethereumToken, uint256 _amountEthereumToken, string memory _destTokenSymbol)
        public
        availableNonce()
    {
        require(symbolToToken[_destTokenSymbol] != address(0) && tokenPairMaps[_ethereumToken] != address(0), "Invalid Token");
        ...
        BandOracleInterface.ReferenceData memory data = bandOracleInterface.getReferenceData(symbol, _destTokenSymbol);
        uint256 amountHarmonyToken = _amountEthereumToken.mul(data.rate);

        lockAndSwapTokenForToken(msg.sender, _harmonyReceiver, _ethereumToken, _amountEthereumToken, _destTokenSymbol, amountHarmonyToken);
    }

    function swapTokenForONE(address _harmonyReceiver, address _ethereumToken, uint256 _amountEthereumToken)
        public
        availableNonce()
    {
        ...
        BandOracleInterface.ReferenceData memory data = bandOracleInterface.getReferenceData(symbol, "ONE");
        uint256 amountONE = _amountEthereumToken.mul(data.rate);

        lockAndSwapTokenForONE(msg.sender, _harmonyReceiver, _ethereumToken, _amountEthereumToken, amountONE);
    }
    ...
    function unlock(
        ...
        if (_token == ETHAddress) {
            ...
            require(IERC20(_token).balanceOf(address(this)) >= _amount, "Insufficient ERC20 token balance for delivery.");
        }
        unlockFunds(_receiver, _token, _amount);
    }
    ...
    function updateFee(uint256 _feeNumerator, uint256 _feeDenominator) public onlyOperator {
        feeNumerator = _feeNumerator;
        feeDenominator = _feeDenominator;
        emit UpdateFee(_feeNumerator, _feeDenominator);
    }

    function withdrawETH(address payable _receiver, uint256 _amountETH) public onlyOperator {
    ...
    function withdrawERC20(address _token, address _receiver, uint256 _amount) public onlyOperator {
    ...
    function checkWithdrawable(address _token, uint256 _amount) internal view returns(bool) {
        if (_token == ETHAddress){
            return address(this).balance - getLockedFunds(_token) >= _amount;
        } else {
            return IERC20(_token).balanceOf(address(this)) - getLockedFunds(_token) >= _amount;
        }
    }
}
